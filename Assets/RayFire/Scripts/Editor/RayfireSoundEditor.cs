...
namespace RayFire
{
    ...
    public class RayfireSoundEditor : Editor
    {
        ...
        void UI_PropsCol()
        {
            ...
            EditorGUI.BeginChangeCheck();
            sound.collision.outputGroup = (AudioMixerGroup)EditorGUILayout.ObjectField (gui_group, sound.collision.outputGroup, typeof(AudioMixerGroup), true);
            if (EditorGUI.EndChangeCheck() == true)
                foreach (RayfireSound scr in targets)
                {
                    scr.collision.outputGroup = sound.collision.outputGroup;
                    SetDirty (scr);
                }
